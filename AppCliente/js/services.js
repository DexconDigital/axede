...
.service('DashService', function($q, $http, USER_ROLES) {
    ...
    return $q(function(resolve, reject) {
      var getsuccess = function(data){
        ...
        if(data.contenido[0] == undefined)
        {
          ...
          if(data.contenido[0].IdProyecto != null ){
            storeUserCredentials(data.contenido[0].Nombre + '.'+ data.contenido[0].Token + '.' + data.contenido[0].Rol + '.' + data.contenido[0].Id + '.' + data.contenido[0].IdProyecto);
          }
          else{
            storeUserCredentials(data.contenido[0].Nombre + '.'+ data.contenido[0].Token + '.' + data.contenido[0].Rol + '.' + data.contenido[0].Id + '.0');
          }
  ...
  return {
    ...
    idProyecto: function() {return idProyecto;},
  };
})
...
.service('AjaxService', function($q, $http){
  ...
  function miAjax(successCall,errorCall,info,tipo){
    ...
    $http({
      method: 'POST',
      // url: 'http://dexcon.aplicando.com.co/php/vistas/'+tipo+'Dexcon.php',
  ...
  function miAjaxLogin(successCall,errorCall,info){
    var data = $.param(info);
    $http({
      method: 'POST',
      // url: 'http://dexcon.aplicando.com.co/php/vistas/Login.php',
  ...
  return {
    miAjax:miAjax,
    miAjaxLogin:miAjaxLogin,
    enviarCorreo:enviarCorreo,
    miAjaxImage:miAjaxImage,
  };
})

.service('ResumenClienteService', function($q, $http){
  ...
  var getPendientesKpi = function(idProyecto, functionAjax){
    return $q(function(resolve, reject) {
      var getsuccess = function(data){
        //console.log(data);
        resolve(data.contenido);
  ...
  return {
    ...
    getPendientesKpi:getPendientesKpi,
    getKpi:getKpi,
  };
})
